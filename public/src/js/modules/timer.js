...
class TimerService {
  ...
  setTodayTime(todaySeconds) {
    ...
    this.updateDisplay();
    logger.log(`Timer set to today's accumulated time: ${todaySeconds} seconds`);
  ...
  start() {
    if (!this.timer) {
      ...
      logger.log("Timer started from", this.seconds, "seconds");
    }
  }
  ...
  stop() {
    if (this.timer) {
      ...
      if (sessionTime > 0 && window.dataService) {
        ...
        logger.log(`Session completed: ${sessionTime} seconds added to daily total`);
  ...
  startAutoSave() {
    this.autoSaveInterval = setInterval(() => {
      ...
      if (this.timer && this.sessionStartTime < this.seconds) {
        const sessionTime = this.seconds - this.sessionStartTime;
        if (sessionTime > 0 && window.dataService) {
          logger.log(`Periodic auto-save: ${sessionTime} seconds`);
          window.dataService.saveDailyTime(sessionTime);
  ...
  autoSaveSession() {
    if (this.timer) {
      const sessionTime = this.seconds - this.sessionStartTime;
      if (sessionTime > 0 && window.dataService) {
        logger.log(`Auto-saving session due to page visibility change: ${sessionTime} seconds`);
        window.dataService.saveDailyTime(sessionTime);
  ...
  autoSaveBeforeClose() {
    if (this.timer) {
      const sessionTime = this.seconds - this.sessionStartTime;
      if (sessionTime > 0) {
        logger.log(`Saving ${sessionTime} seconds to localStorage before page close`);
        logger.log(`Total time at close: ${this.seconds} seconds`);
        ...
        if (window.dataService) {
          try {
            ...
            logger.log('Firebase save failed, localStorage backup will handle recovery');
          }
        }
      }
    } else {
      // Even if timer is not running, save current state as backup
      logger.log(`Page closing with timer at ${this.seconds} seconds (not running)`);
    }
  }
    ...
    if (unsavedData) {
      try {
        ...
        if (data.date === today && data.sessionTime > 0 && timeAge < maxAge) {
          logger.log(`Recovering ${data.sessionTime} seconds from previous session`);
          ...
          logger.log(`Timer updated to show ${this.seconds} total seconds including recovered time`);
        } else {
          // Clear old or invalid data
          localStorage.removeItem('unsavedTime');
          logger.log('Cleared old unsaved time data');
        }
      } catch (error) {
        logger.error('Error recovering unsaved time:', error);
        localStorage.removeItem('unsavedTime');
      }
    }
  }
  ...
  checkDayChange() {
    ...
    if (this.currentDate !== today) {
      logger.log(`Day changed from ${this.currentDate} to ${today}`);
  ...
  checkLocalStorageBackup() {
    const unsavedData = localStorage.getItem('unsavedTime');
    if (unsavedData) {
      logger.log('LocalStorage backup found:', JSON.parse(unsavedData));
      return JSON.parse(unsavedData);
    } else {
      logger.log('No localStorage backup found');
      return null;
    }
  }
